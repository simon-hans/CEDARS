...
standardize_nlp <- function(input_df, engine) {
    ...
    if (engine == "udpipe") {
        ...
        output_df <- data.frame(text_id = input_df$doc_id, paragraph_id = input_df$paragraph_id, sentence_id = input_df$sentence_id, start = input_df$start, end = input_df$end, token = input_df$token,
            lemma = input_df$lemma, token_id = input_df$token_id, upos = input_df$upos, xpos = input_df$xpos, head_token_id = input_df$head_token_id, dependency = input_df$dep_rel, features = input_df$feats)
    }
...
save_credentials <- function(user, password, host, destination_path = getwd()) {
    ...
    app_path <- paste(find.package("CEDARS", lib.loc = NULL, quiet = TRUE), "/shiny/CEDARS.R", sep = "")

    g_user <- user
    g_password <- password
    g_host <- host

    file.copy(from = app_path, to = paste(destination_path, "/CEDARS.R", sep = ""))
