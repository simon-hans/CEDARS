...
pre_search <- function(patient_vect = NA, uri_fun, user, password, host, database) {
    ...
    for (i in 1:length_list) {
        # Records for this patient undergo admin lock during the upload But first, old user-locked records are
        # unlocked A record is considered open for annotation if admin lock was successful
        ...
        if (open == TRUE) {
            ...
            if (length(unique_sentences[,1]) > 0) {
                ...
                retained_fields <- c("patient_id", "doc_id", "text_sequence", "paragraph_id", "sentence_id",
                  "text_date", "selected", "note_text", "text_tag_1", "text_tag_2", "text_tag_3", "text_tag_4",
                ...
                unique_sentences <- unique_sentences[order(unique_sentences$text_date, unique_sentences$doc_id, unique_sentences$text_sequence,
                    unique_sentences$paragraph_id, unique_sentences$sentence_id, decreasing = FALSE, method = "radix"), ]
                unique_sentences$unique_id <- 1:length(unique_sentences[, 1])
                ...
                unique_sentences$note_text <- sapply(unique_sentences$doc_id, aggregate_note, annotations, parse_result$cui_elements)
