...
get_wrapper <- function(database, end_user, end_user_password, html = TRUE, position, patient_id = NA) {

    get_data(g_mongodb_uri_fun, g_user, g_password, g_host, database, end_user, end_user_password, html, position, patient_id)
...
post_wrapper <- function(database, end_user, end_user_password, position, event_date, pt_comments) {

    post_data(g_mongodb_uri_fun, g_user, g_password, g_host, database, end_user, end_user_password, position, event_date, pt_comments)
...
get_data <- function(uri_fun, user, password, host, database, end_user, end_user_password, html = TRUE, position, patient_id = NA) {

    if (password_verification(uri_fun, user, password, host, database, end_user, end_user_password) == TRUE) {

        patients_con <- mongo_connect(uri_fun, user, password, host, database, "PATIENTS")

        # If end user specified a desired patient record, we try to commit to it, if not found or locked we return an error.
        ...
        if (!is.na(patient_id)) {
            ...
            if (dim(patient)[1] > 0) {
                ...
                committed <- commit_patient(uri_fun, user, password, host, database, end_user, search_query, use_negation, hide_duplicates, patient_id)
                ...
                if (committed == TRUE) {
                  ...
                  if (length(sentences_df[, 1]) > 0) {
                    ...
                    # error_4 = no sentences to evaluate
                  } else out <- "error_4"

                  # error_3 = patient locked by another user
                } else out <- "error_3"
            ...
            # Assess if there is already a commited patient for this end user
            query <- paste("{ \"end_user\" : ", "\"", end_user, "\", ", " \"locked\" : true}", sep = "")
            ...
            if (length(previously_selected) == 0) {
                ...
                while (committed == FALSE) {
                  committed <- commit_patient(uri_fun, user, password, host, database, end_user, search_query, use_negation, hide_duplicates)
                }
            ...
            if (length(previously_selected) != 0) {
                ...
                out <- list()
                out$patient_id <- previously_selected$patient_id
                out$event_date <- previously_selected$event_date
                out$max_unique_id <- max_unique_id
                out$pt_comments <- previously_selected$pt_comments

                # If end user already had locked record requested position provided, corresponding sentence resulted Otherwise, otherwise first unreviewed sentence in (newly or not) locked record is resulted
                if (previous_exists == TRUE & !is.na(position))
                  out <- append(out, as.list(subset(sentences_df_ori, unique_id == position))) else out <- append(out, as.list(sentences_df[1, ]))
    ...
    if (out[1] != "error_0" & out[1] != "error_1" & out[1] != "error_2" & out[1] != "error_3" & html == TRUE)
        out <- colorize(out)
...
post_data <- function(uri_fun, user, password, host, database, end_user, end_user_password, position, event_date, pt_comments) {
    ...
    if (password_verification(uri_fun, user, password, host, database, end_user, end_user_password) == TRUE) {
        ...
        if (length(selected_patient) > 0) {
            ...
            query <- paste("{ \"end_user\" : ", "\"", end_user, "\"", " , \"locked\" : true, \"patient_id\" : ", selected_patient, "}", sep = "")
            ...
            if (is.na(event_date))
                update_value <- paste("{\"$set\":{\"sentences\": ", jsonlite::toJSON(sentences), " , \"pt_comments\" : ", "\"", pt_comments, "\" }}", sep = "") else {

                if (event_date != "DELETE")
                  update_value <- paste("{\"$set\":{\"sentences\": ", jsonlite::toJSON(sentences), ", \"event_date\" : ", "\"", event_date, "\"", " , \"pt_comments\" : ", "\"", pt_comments, "\" }}", sep = "") else {

                  update_value <- paste("{\"$set\":{\"sentences\": ", jsonlite::toJSON(sentences), ", \"event_date\" : null", " , \"pt_comments\" : ", "\"", pt_comments, "\" }}", sep = "")
...
password_verification <- function(uri_fun, user, password, host, database, end_user, end_user_password) {
    ...
    query_value <- paste("{ \"user\" : ", "\"", end_user, "\" , \"password\" : ", "\"", end_user_password, "\" }", sep = "")
...
commit_patient <- function(uri_fun, user, password, host, database, end_user, search_query, use_negation, hide_duplicates, patient_id = NA) {

    committed <- TRUE

    patients_con <- mongo_connect(uri_fun, user, password, host, database, "PATIENTS")

    sentences <- get_patient(uri_fun, user, password, host, database, end_user, search_query, use_negation, hide_duplicates, patient_id)

    # If there are no sentences after inital search, we keep looking.
    # This doe snot apply if patient ID for search wa specified.
    ...
    if (is.data.frame(sentences)) {
        ...
        if (length(sentences[, 1]) > 0) {
            ...
            retained_fields <- c("patient_id", "doc_id", "text_sequence", "paragraph_id", "sentence_id", "text_date", "selected", "note_text", "text_tag_1", "text_tag_2", "text_tag_3", "text_tag_4", "text_tag_5",
            ...
            patient_id <- sentences$patient_id[1]
            if (is.na(patient_id) | !("reviewed" %in% colnames(sentences))) sentences$reviewed <- rep(FALSE, length(sentences[, 1]))
            sentences$text_date <- as.Date(sentences$text_date)
            ...
            # If all dates for sentences left to evaluate are after previously reviewed sentences, we close the case and commit to another patient.
            # This does not apply if patient record was subject of a direct search!

            event_date <- patients_con$find(query = query, fields = "{ \"event_date\" : 1 , \"_id\" : 0 }")$event_date

            if (is.na(patient_id) & !is.null(event_date)) {
...
get_patient <- function(uri_fun, user, password, host, database, end_user, search_query, use_negation, hide_duplicates, patient_id = NA) {
    ...
    if (is.na(selected_patient))
        ...
        # If there are no prior sentences, we compute them; else, if NLP annotations were not updated and there are already sentences, we use them If NLP annotations were updated and there are prior
        # sentences, we get the old ones and merge with the new
        ...
        if (length(patient_info$sentences[[1]][, 1]) == 0) {
            ...
            # If still no sentences, we close and recurse
            if (length(sentences[, 1] > 0))
            ...
            if (patient_info$updated == TRUE) {
                ...
                # If there is an existing sentences dataframe it is merged into the new one, so as to keep any human-entered annotations Updated sets always include the older ones, so it might have earlier versions
                # of one sentence but not vice versa, and we always keep the extra info from the update, so some identical sentences might exist with different dates
                ...
                if (length(new_sentences[, 1] > 0)) {
                  ...
                  new_sentences$note_text <- sapply(new_sentences$doc_id, aggregate_note, annotations, parse_result$keyword_elements, parse_result$cui_elements)
                  sentences$selected <- as.character(sentences$selected)
                  new_sentences$reviewed <- NULL
                  new_sentences$unique_id <- NULL
                  new_sentences$patient_id <- NULL
                  sentences <- merge(new_sentences, sentences, by = c("doc_id", "text_sequence", "paragraph_id", "sentence_id", "text_date", "selected", "note_text"), all.x = TRUE, all.y = TRUE)
...
aggregate_note <- function(selected_doc_id, annotations, cui_elements) {
    ...
    note_df <- note_df[order(note_df$text_sequence, note_df$text_id, note_df$paragraph_id, note_df$sentence_id, note_df$token_id, decreasing = FALSE, method = "radix"), ]
...
select_patient <- function(uri_fun, user, password, host, database, end_user, patient_id = NA) {
    ...
    # If specific patient ID is provided, we unlock any other prior locked record and attempt to lock the desired record
    if (!is.na(patient_id)) {
    ...
    # If end user already had selected a patient but did not finish the case, the lock date will be reset and same patient returned

    query <- paste("{ \"end_user\" : ", "\"", end_user, "\", ", " \"locked\" : true}", sep = "")
    update_value <- paste("{\"$set\":{\"time_locked\": {\"$date\" : ", "\"", strftime(Sys.time(), "%Y-%m-%dT%H:%M:%SZ", "UTC"), "\"", "}}}", sep = "")
    patients_con$update(query, update_value)
    previously_selected <- patients_con$find(query)
...
lock_records <- function(uri_fun, user, password, host, database, end_user, patient_id = NA) {
    ...
    if (is.na(patient_id))
        query <- paste("{ \"$and\": [ {\"$or\": [{ \"reviewed\" : false }, { \"updated\" : true }] } , {\"locked\" : false }, {\"admin_locked\" : false }] }", sep = "") else query <- paste("{ \"$and\": [{\"locked\" : false }, {\"admin_locked\" : false }, {\"patient_id\" : ", patient_id, "}] }", sep = "")

    update_value <- paste("{\"$set\" : {\"locked\": true , \"end_user\" : ", "\"", end_user, "\"", ", \"time_locked\": { \"$date\" : ", "\"", strftime(Sys.time(), "%Y-%m-%dT%H:%M:%SZ", "UTC"), "\"", "}}}",
        sep = "")
...
lock_records_admin <- function(uri_fun, user, password, host, database, patient_id) {
    ...
    # If the record locked for admin or patient does not exist, output is true If record exists AND could not be locked for admin, output is false
    if (length(result) > 0)
...
unlock_records <- function(uri_fun, user, password, host, database) {
    ...
    query <- paste("{ \"time_locked\" : { \"$lt\" : { \"$date\" : ", "\"", strftime(Sys.time() - 86400, "%Y-%m-%dT%H:%M:%SZ", "UTC"), "\"", "}}}", sep = "")
