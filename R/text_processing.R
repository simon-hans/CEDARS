...
document_processor <- function(text_df, text_format, nlp_engine, negex_simp, negex_depth, single_core_model = NA) {
    ...
    if (nlp_engine == "udpipe") {
        annotated_text <- udpipe::udpipe_annotate(nlp_model, text, doc_id = text_id, tokenizer = "tokenizer",
...
patient_processor_par <- function(cl, sub_corpus, text_format, nlp_engine, negex_simp, umls_selected, max_n_grams_length,
    ...
    if (length(sub_corpus_short[, 1]) > 0) {
        ...
        if (!is.na(cl[1]))
            ...
            annotations <- lapply(sub_corpus_short, document_processor, text_format, nlp_engine, negex_simp,
                negex_depth, single_core_model)
...
batch_processor_db <- function(patient_vect, text_format, nlp_engine, URL, negex_simp, umls_selected, uri_fun,
    ...
    # We create a computing cluster If requested # of cores > available minus one, will use available minus one
    ...
    if (!is.na(cl[1])) {
        ...
        parallel::clusterExport(cl, c("sanitize", "standardize_nlp", "negation_tagger", "negex_token_tagger",
            "id_expander", "negex_processor", "negex_simp", "negex_depth", "URL"), envir = environment())
    ...
    for (i in 1:length_list) {
        ...
        # unlocked A record is considered open for annotation if 1) the patient is not in the roster yet or 2) admin
        # lock was successful
