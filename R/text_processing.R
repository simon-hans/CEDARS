...
document_processor <- function(text_df, text_format, nlp_engine, negex_simp, negex_depth) {
...
patient_processor_par <- function(cl, sub_corpus, text_format, nlp_engine, negex_simp, umls_selected, max_n_grams_length, negex_depth) {
    ...
    if (!is.na(cl[1])) 
        annotations <- parallel::parLapply(cl, sub_corpus_short, document_processor, text_format, nlp_engine, negex_simp, negex_depth) else {
        
        annotations <- lapply(sub_corpus_short, document_processor, text_format, nlp_engine, negex_simp, negex_depth, nlp_model)
...
batch_processor_db <- function(patient_vect, text_format, nlp_engine, URL, negex_simp, umls_selected, uri_fun, user, password, host, database, max_n_grams_length, negex_depth, select_cores) {
    ...
    if (is.na(select_cores) | select_cores > no_cores | select_cores < 1) 
        ...
        if (select_cores > 1) 
            cl <- parallel::makeCluster(select_cores) else cl <- NA
